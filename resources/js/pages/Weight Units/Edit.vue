<template>
  <div v-if="show" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
    <div class="bg-white p-6 rounded shadow w-full max-w-md">
      ...
      <!-- Minimum Quantity -->
      <label class="block text-sm mb-1">Minimum Quantity</label>
      <input
      ...
      />
      <p v-if="form.errors.minimum_quantity" class="text-red-600 text-sm mt-1">
...
watch(
  ...
  (newUnit) => {
    if (newUnit) {
      ...
      form.minimum_quantity = newUnit.minimum_quantity;
      form.quantity = newUnit.quantity;
    }
  },
  { immediate: true }
);
